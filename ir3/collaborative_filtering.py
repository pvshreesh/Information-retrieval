import time
import numpy as np
from similarity_funcs import pearson_sim as sim
...
class Collaborate:
    ...
    def predictor(self, user, item, k=2, baseline=False):
        """
    ...
    def comp(self, k=2, baseline=False):
        """
        ...
        for i in range(self.M.shape[0]):
            for j in range(self.M.shape[1]):
                if self.M[i, j] == 0:
                    filled[i, j] = self.predictor(j, i, k=k, baseline=baseline)
                else:
